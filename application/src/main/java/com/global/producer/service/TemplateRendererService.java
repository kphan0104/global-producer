...
public class TemplateRendererService {
    ...
    public String render(FlowDefinition flowDefinition, Path messageFile, Instant now) {
        ...
        String renderedTimestamp = compiledFlowDefinition.timestampFormatter().format(now);
        ...
        for (CompiledTemplatePart templatePart : compiledTemplate.parts()) {
            ...
            if (templatePart == TimestampPlaceholderPart.INSTANCE) {
                renderedContent.append(renderedTimestamp);
                continue;
            }
    ...
    private CompiledFlowDefinition compileFlowDefinition(FlowDefinition flowDefinition) {
        DateTimeFormatter timestampFormatter = timestampFormatter(flowDefinition.getTimestamp());
        ...
        flowDefinition.getMessageFiles().forEach(messageFile -> {
            ...
            CompiledTemplate compiledTemplate = compileTemplate(flowDefinition, messageFile, template);
            compiledTemplates.put(messageFile.normalize(), compiledTemplate);
        });

        return new CompiledFlowDefinition(timestampFormatter, Map.copyOf(compiledTemplates));
    }
    ...
    private CompiledTemplate compileTemplate(FlowDefinition flowDefinition, Path messageFile, String template) {
        ...
        while (matcher.find()) {
            ...
            parts.add(resolvePlaceholderPart(flowDefinition, messageFile, matcher.group(1)));
    ...
    private CompiledTemplatePart resolvePlaceholderPart(
        ...
        if (TIMESTAMP_PLACEHOLDER.equals(placeholder)) {
            return TimestampPlaceholderPart.INSTANCE;
        }
    ...
    private DateTimeFormatter timestampFormatter(TimestampDefinition timestampDefinition) {
        ...
        if ("ISO8601".equalsIgnoreCase(format) || "ISO_OFFSET_DATE_TIME".equalsIgnoreCase(format)) {
            return DateTimeFormatter.ISO_OFFSET_DATE_TIME.withLocale(locale).withZone(zoneId);
        }
        if ("ISO_INSTANT".equalsIgnoreCase(format)) {
            return DateTimeFormatter.ISO_INSTANT.withLocale(locale).withZone(zoneId);
        }
    ...
    private enum TimestampPlaceholderPart implements CompiledTemplatePart {
        INSTANCE
    }
